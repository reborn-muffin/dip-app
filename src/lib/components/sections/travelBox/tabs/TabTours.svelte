<script>
  ...
  const handleSearch = async () => {
    tours = await fetchTours(changedFields.map((f) => f.rule).join(' && '), false)
  }
  ...
  <div class="tab-para">
    ...
    {#if tours.length > 0}
      <div style="margin-top: 20px; overflow: scroll; max-height: 230px">
        <h2>Found {tours.length} tours</h2>
        <ul style="margin-top: 10px; display: flex; flex-direction: column; gap: 4px">
          {#each tours as tour}
            <li style="border: rgba(20,176,243,0.37) solid 2px; border-radius: 4px; padding: 2px 8px; display: flex; justify-content: space-between">
              <div>
                Price: {tour.price}
                Check In date: {formatDate(new Date(tour.checkInDate))}
                Check Out date: {formatDate(new Date(tour.checkOutDate))}
              </div>
